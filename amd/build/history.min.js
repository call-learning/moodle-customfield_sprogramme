...
/**
   ...
   */Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.default=void 0,_state=_interopRequireDefault(_state),_repository=_interopRequireDefault(_repository),_notification=_interopRequireDefault(_notification),_history=_interopRequireDefault(_history);class History{constructor(rfcid,datafieldid){this.rfcid=rfcid,this.datafieldid=datafieldid,this.getProgrammeHistory()}async getProgrammeHistory(){const context={rfcid:this.rfcid,datafieldid:this.datafieldid};try{const response=await _repository.default.getProgrammeHistory(context);0===response.length&&_state.default.setValue("history",[]);const history={modules:this.parseModules(response),columns:response.columns,rfcs:response.rfcs};_state.default.setValue("history",history)}catch(error){_notification.default.exception(error)}}parseModules(response){return response.modules.forEach((mod=>{mod.rows.map((row=>(row.cells=row.cells.map((cell=>{const column=response.columns.find((column=>column.column==cell.column));return(cell=Object.assign({},cell,column)).changed=cell.value!==cell.oldvalue,cell})),row)))})),response.modules}}var _default={init:(rfcid,datafieldid)=>((0,_history.default)(),new History(rfcid,datafieldid))};return _exports.default=_default,_exports.default}));
