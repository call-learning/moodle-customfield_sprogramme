...
use customfield_sprogramme\external\reject_rfc;
 ...
 * @covers \customfield_sprogramme\external\reject_rfc
 */
final class reject_rfc_test extends \externallib_advanced_testcase {
    /**
     * Helper
     *
     * @param mixed ...$params
     * @return mixed
     */
    protected function reject_rfc(...$params) {
        $rejectrfc = reject_rfc::execute(...$params);
        return external_api::clean_returnvalue(reject_rfc::execute_returns(), $rejectrfc);
    }
    ...
    public function test_execute_wrong_parameters(): void {
        ...
        $this->reject_rfc(1234, 5678);

        $this->setAdminUser();
        $this->expectException(\invalid_parameter_exception::class);
        $this->reject_rfc(1234, get_admin()->id);
    }
    ...
    public function test_with_wrong_role(): void {
        ...
        $this->expectExceptionMessage('customfield_sprogramme/rfcrejectionnotallowed');
        $this->setUser($user3);
        $this->reject_rfc($cfdataid, $users[1]->id);
    }
