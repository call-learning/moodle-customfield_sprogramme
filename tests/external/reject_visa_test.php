...
/**
 * Tests for the reject_visa class.
 *
 * @package    customfield_sprogramme
 * @category   test
 * @copyright  2026 CALL Learning <laurent@call-learning.fr>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @covers \customfield_sprogramme\external\reject_visa
 */
final class reject_visa_test extends \externallib_advanced_testcase {
    /**
     * Helper.
     *
     * @param mixed ...$params
     * @return mixed
     */
    protected function reject_visa(...$params) {
        $rejected = reject_visa::execute(...$params);
        return external_api::clean_returnvalue(reject_visa::execute_returns(), $rejected);
    }
    ...
    public function test_execute(): void {
        ...
        $result = $this->reject_visa($rfcid, 'Not aligned');
        $this->assertTrue($result);
        $visa = sprogramme_visa::get_record(['rfcid' => $rfcid, 'visauser' => $reviewer->id]);
        $this->assertNotEmpty($visa);
        $this->assertEquals(sprogramme_visa::STATUS_REJECTED, $visa->get('status'));
        $this->assertEquals('Not aligned', $visa->get('comment'));
    }
    ...
    public function test_execute_with_wrong_role(): void {
        ...
        $this->expectExceptionMessage('customfield_sprogramme/visaeditionnotallowed');
        $this->reject_visa($rfcid, 'Nope');
    }
